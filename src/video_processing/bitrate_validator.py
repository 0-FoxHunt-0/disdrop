...
class BitrateValidator:
    ...
    def _load_encoder_minimums(self) -> Dict[str, int]:
        ...
        # Default minimums based on encoder capabilities (raised for better audio quality)
        defaults = {
            'libx264': 10,      # kbps - FFmpeg libx264 minimum
            'libx265': 15,      # kbps - HEVC requires higher minimum
            'h264_nvenc': 8,   # kbps - NVENC minimum
            'h264_amf': 8,     # kbps - AMD AMF minimum
            'h264_qsv': 8,     # kbps - Intel QSV minimum
            'h264_videotoolbox': 8,  # kbps - Apple VideoToolbox minimum
        }
    ...
    def validate_bitrate(self, bitrate_kbps: int, encoder: str) -> ValidationResult:
        ...
        min_required = self.encoder_minimums.get(encoder, 10)  # Default to 10kbps if unknown (libx264 minimum)
    ...
    def suggest_adjustments(self, params: Dict[str, Any], target_size_mb: float, 
        ...
        min_bitrate = self.encoder_minimums.get(encoder, 10)
    ...
    def get_encoder_minimum(self, encoder: str) -> int:
        ...
        return self.encoder_minimums.get(encoder, 10)
