...
class AdvancedGifOptimizer:
    def __init__(self, config_manager, shutdown_checker: Optional[Callable[[], bool]] = None):
        ...
        # Shared palette cache directory (same as GifGenerator)
    ...
    def _get_palette_cache_key(self, input_video: str, method: str, width: int, height: int, 
                               fps: int, colors: int, start_time: float = 0, duration: float = 0) -> str:
        """Generate cache key for palette file based on input parameters"""
        try:
            # Use exact parameters for cache precision (avoid false cache hits)
            # Only normalize if explicitly needed for cache efficiency
            fps_normalized = float(fps)  # Use exact fps to avoid false cache hits
            colors_normalized = colors  # Use exact colors to avoid false cache hits
            
            # Get file modification time for cache invalidation
            safe_path = input_video
            try:
                mtime = os.path.getmtime(safe_path) if os.path.exists(safe_path) else 0
            except Exception:
                mtime = 0
            ...
            key_payload = {
                ...
                # Include mpdecimate settings for cache precision (method-specific defaults)
                'mpdecimate_hi': 512,
                'mpdecimate_lo': 256,
                'mpdecimate_frac': 0.3,
            }
            ...
            return self._create_unique_temp_filename(f"{method}_palette", ".png")
    ...
    def optimize_gif(self, gif_path: str, max_size_mb: float) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Optimizing GIF: current={original_size_mb:.2f}MB, target={target_size_mb:.2f}MB"
            )
            logger.debug(f"Starting optimization: original={original_size_mb:.2f}MB, target={target_size_mb:.2f}MB, target_bytes={target_bytes}")
            ...
            logger.debug(f"Gifsicle available: {gifsicle_available}")
            ...
            logger.debug(f"Original over ratio: {original_over_ratio:.2%}, far_over_skip: {far_over_skip}, skip_threshold: {self.skip_gifsicle_far_over_ratio:.2%}")
            ...
            if gifsicle_available and not far_over_skip:
                attempt_number += 1
                logger.debug(f"[Stage 1] Attempt {attempt_number}: Lossless gifsicle optimization")
                ...
                if self._gifsicle_lossless_optimize(gif_path, temp_lossless) and os.path.exists(temp_lossless):
                    temp_size_mb = os.path.getsize(temp_lossless) / 1024 / 1024
                    logger.debug(f"[Stage 1] Lossless optimization result: {temp_size_mb:.2f}MB")
                    try:
                        os.replace(temp_lossless, gif_path)
                        optimization_attempts.append({
                            'stage': 'Stage 1: Lossless gifsicle',
                            'attempt': attempt_number,
                            'result_size_mb': temp_size_mb,
                            'success': True
                        })
                    except Exception as replace_e:
                        logger.warning(f"Failed to replace with lossless optimized file: {replace_e}")
                        optimization_attempts.append({
                            'stage': 'Stage 1: Lossless gifsicle',
                            'attempt': attempt_number,
                            'result_size_mb': temp_size_mb,
                        ...
                        try:
                            os.remove(temp_lossless)
                        except Exception:
                            pass
                        temp_lossless = None
                else:
                    logger.debug(f"[Stage 1] Lossless optimization failed or no output file")
                    optimization_attempts.append({
                        'stage': 'Stage 1: Lossless gifsicle',
                        'attempt': attempt_number,
                        'success': False,
                        'error': 'Optimization failed or no output'
                    })
            ...
            logger.debug(f"[Stage 1] After lossless: {current_size_mb:.2f}MB (target: {target_size_mb:.2f}MB)")
            if current_bytes <= target_bytes:
                if backup_path and os.path.exists(backup_path):
                    try:
                        ...
                        pass
                logger.info(f"GIF under target after lossless optimization: {current_size_mb:.2f}MB <= {target_size_mb:.2f}MB")
                logger.debug(f"Optimization summary: {len(optimization_attempts)} attempts, final size: {current_size_mb:.2f}MB")
                return True
            ...
            logger.debug(f"Over ratio: {over_ratio:.2%}, near_target: {near_target}, current: {current_size_mb:.2f}MB, target: {target_size_mb:.2f}MB")
            ...
            if gifsicle_available and near_target:
                attempt_number += 1
                logger.debug(f"[Stage 2] Attempt {attempt_number}: Bounded gifsicle search (near target)")
                temp_best = gif_path + ".near.tmp.gif"
                if self._bounded_gifsicle_near_target(gif_path, temp_best, target_bytes):
                    temp_size_mb = os.path.getsize(temp_best) / 1024 / 1024 if os.path.exists(temp_best) else 0
                    logger.debug(f"[Stage 2] Bounded gifsicle result: {temp_size_mb:.2f}MB")
                    try:
                        os.replace(temp_best, gif_path)
                        temp_best = None  # Mark as replaced, don't clean up
                        optimization_attempts.append({
                            'stage': 'Stage 2: Bounded gifsicle',
                            'attempt': attempt_number,
                            'result_size_mb': temp_size_mb,
                            'success': True
                        })
                    except Exception as replace_e:
                        logger.warning(f"Failed to replace with bounded gifsicle result: {replace_e}")
                        optimization_attempts.append({
                            'stage': 'Stage 2: Bounded gifsicle',
                            'attempt': attempt_number,
                            'result_size_mb': temp_size_mb,
                        ...
                        current_bytes = os.path.getsize(gif_path)
                        current_size_mb = current_bytes / 1024 / 1024
                        logger.debug(f"[Stage 2] After bounded gifsicle: {current_size_mb:.2f}MB (target: {target_size_mb:.2f}MB)")
                        if current_bytes <= target_bytes:
                            if backup_path and os.path.exists(backup_path):
                                try:
                                    os.remove(backup_path)
                                    ...
                                    pass
                            logger.info(f"GIF met target via bounded gifsicle search: {current_size_mb:.2f}MB <= {target_size_mb:.2f}MB")
                            logger.debug(f"Optimization summary: {len(optimization_attempts)} attempts, final size: {current_size_mb:.2f}MB")
                            return True
                else:
                    logger.debug(f"[Stage 2] Bounded gifsicle search failed")
                    optimization_attempts.append({
                        'stage': 'Stage 2: Bounded gifsicle',
                        'attempt': attempt_number,
                        'success': False,
                ...
                # Cleanup temp file if it wasn't used
                if temp_best and os.path.exists(temp_best):
                    try:
                        os.remove(temp_best)
                    except Exception:
                        pass
                ...
                if not gifsicle_available:
                    logger.debug(f"[Stage 2] Skipped - gifsicle not available")
                elif not near_target:
                    logger.debug(f"[Stage 2] Skipped - not near target (over_ratio: {over_ratio:.2%})")
            ...
            original_width = info.get('width', 320)
            original_fps = info.get('fps', 12)
            new_width = max(2, int((original_width * scale_factor) // 2 * 2))
            if enforce and min_w > 0 and new_width < min_w:
                new_width = int(min_w // 2 * 2)
            fps = max(6, min(15, int(round(original_fps))))
            if enforce and min_fps > 0 and fps < min_fps:
                fps = min_fps

            attempt_number += 1
            logger.debug(f"[Stage 3] Attempt {attempt_number}: FFmpeg palette re-encode - width={new_width} (from {original_width}), fps={fps} (from {original_fps:.1f}), colors=256, scale_factor={scale_factor:.2f}")
            logger.debug(f"[Stage 3] Before re-encode: {current_size_mb:.2f}MB (target: {target_size_mb:.2f}MB)")

            if self._shutdown_checker():
                return False
            if self._ffmpeg_palette_reencode(gif_path, temp_ffmpeg, new_width, fps, max_colors=256):
                temp_size_mb = os.path.getsize(temp_ffmpeg) / 1024 / 1024 if os.path.exists(temp_ffmpeg) else 0
                logger.debug(f"[Stage 3] FFmpeg re-encode result: {temp_size_mb:.2f}MB")
                try:
                    os.replace(temp_ffmpeg, gif_path)
                    temp_ffmpeg = None  # Mark as replaced, don't clean up
                    optimization_attempts.append({
                        'stage': 'Stage 3: FFmpeg re-encode',
                        'attempt': attempt_number,
                        'result_size_mb': temp_size_mb,
                        'params': {'width': new_width, 'fps': fps, 'colors': 256},
                        'success': True
                    })
                except Exception as replace_e:
                    logger.warning(f"Failed to replace with ffmpeg re-encoded file: {replace_e}")
                    optimization_attempts.append({
                        'stage': 'Stage 3: FFmpeg re-encode',
                        'attempt': attempt_number,
                        'result_size_mb': temp_size_mb,
                    ...
                    current_bytes = os.path.getsize(gif_path) if os.path.exists(gif_path) else original_bytes
                    current_size_mb = current_bytes / 1024 / 1024
                else:
                    current_bytes = os.path.getsize(gif_path)
                    current_size_mb = current_bytes / 1024 / 1024
                    logger.debug(f"[Stage 3] After re-encode: {current_size_mb:.2f}MB (target: {target_size_mb:.2f}MB)")
                ...
                if current_bytes > target_bytes and gifsicle_available and not self._shutdown_checker():
                    logger.debug(f"[Stage 4] Attempting gifsicle squeeze for small overage")
                    if self._gifsicle_squeeze_small_overage(gif_path):
                        current_bytes = os.path.getsize(gif_path)
                        current_size_mb = current_bytes / 1024 / 1024
                        logger.debug(f"[Stage 4] After gifsicle squeeze: {current_size_mb:.2f}MB")
                ...
                if current_bytes <= target_bytes:
                    if backup_path and os.path.exists(backup_path):
                        try:
                            ...
                            pass
                    logger.info(f"GIF met target via ffmpeg re-encode: {current_size_mb:.2f}MB <= {target_size_mb:.2f}MB")
                    logger.debug(f"Optimization summary: {len(optimization_attempts)} attempts, final size: {current_size_mb:.2f}MB")
                    return True
            else:
                logger.debug(f"[Stage 3] FFmpeg re-encode failed")
                optimization_attempts.append({
                    'stage': 'Stage 3: FFmpeg re-encode',
                    'attempt': attempt_number,
                    'params': {'width': new_width, 'fps': fps, 'colors': 256},
            ...
            if current_bytes > target_bytes and not self._shutdown_checker():
                logger.debug(f"[Iterative Tightening] Starting iterative tightening: current={current_size_mb:.2f}MB, target={target_size_mb:.2f}MB")
                # Expanded color reduction attempts
                color_attempts = [192, 160, 128, 96, 64]
                max_iterations = 10
                iteration_count = 0
                
                # Start with reduced width and fps
                ...
                # Try multiple color values with progressive width/fps reduction
                for color_idx, colors in enumerate(color_attempts):
                    if iteration_count >= max_iterations:
                        logger.debug(f"[Iterative Tightening] Reached max iterations ({max_iterations}), stopping")
                        break
                    ...
                    attempt_number += 1
                    iteration_count += 1
                    logger.debug(f"[Iterative Tightening] Iteration {iteration_count}/{max_iterations}, Attempt {attempt_number}: width={attempt_width}, fps={attempt_fps}, colors={colors}")
                    logger.debug(f"[Iterative Tightening] Before iteration: {current_size_mb:.2f}MB (target: {target_size_mb:.2f}MB)")
                    ...
                    if self._ffmpeg_palette_reencode(gif_path, temp_try, attempt_width, attempt_fps, max_colors=colors):
                        temp_size_mb = os.path.getsize(temp_try) / 1024 / 1024 if os.path.exists(temp_try) else 0
                        logger.debug(f"[Iterative Tightening] Re-encode result: {temp_size_mb:.2f}MB")
                        try:
                            os.replace(temp_try, gif_path)
                            temp_try = None  # Mark as replaced, don't clean up
                            current_bytes = os.path.getsize(gif_path)
                            current_size_mb = current_bytes / 1024 / 1024
                            logger.debug(f"[Iterative Tightening] After replace: {current_size_mb:.2f}MB (target: {target_size_mb:.2f}MB)")
                            
                            optimization_attempts.append({
                                'stage': f'Iterative Tightening (iteration {iteration_count})',
                                'attempt': attempt_number,
                                'result_size_mb': temp_size_mb,
                                'params': {'width': attempt_width, 'fps': attempt_fps, 'colors': colors},
                                'success': True
                            })
                            
                            # Try gifsicle squeeze if still over target
                            if current_bytes > target_bytes and gifsicle_available and not self._shutdown_checker():
                                logger.debug(f"[Iterative Tightening] Attempting gifsicle squeeze after iteration {iteration_count}")
                                if self._gifsicle_squeeze_small_overage(gif_path):
                                    current_bytes = os.path.getsize(gif_path)
                                    current_size_mb = current_bytes / 1024 / 1024
                                    logger.debug(f"[Iterative Tightening] After squeeze: {current_size_mb:.2f}MB")
                            ...
                            if current_bytes <= target_bytes:
                                if backup_path and os.path.exists(backup_path):
                                    try:
                                        ...
                                        pass
                                logger.info(f"GIF met target via iterative ffmpeg re-encode (iteration {iteration_count}): {current_size_mb:.2f}MB <= {target_size_mb:.2f}MB")
                                logger.debug(f"Optimization summary: {len(optimization_attempts)} attempts, final size: {current_size_mb:.2f}MB")
                                return True
                            
                            # If we've tried all colors at this width/fps, reduce further for next color set
                            if color_idx == len(color_attempts) - 1:
                                ...
                                if enforce:
                                    ...
                                    if min_fps > 0 and attempt_fps < min_fps:
                                        attempt_fps = min_fps
                                logger.debug(f"[Iterative Tightening] Reduced params for next round: width={attempt_width}, fps={attempt_fps}")
                        except Exception as replace_e:
                            logger.warning(f"Failed to replace with iterative re-encoded file: {replace_e}")
                            optimization_attempts.append({
                                'stage': f'Iterative Tightening (iteration {iteration_count})',
                                'attempt': attempt_number,
                                'result_size_mb': temp_size_mb,
                        ...
                        logger.debug(f"[Iterative Tightening] Re-encode failed for iteration {iteration_count}")
                        optimization_attempts.append({
                            'stage': f'Iterative Tightening (iteration {iteration_count})',
                            'attempt': attempt_number,
                            'params': {'width': attempt_width, 'fps': attempt_fps, 'colors': colors},
                        ...
                        # Clean up temp file if re-encode failed
                        try:
                            if os.path.exists(temp_try):
                                os.remove(temp_try)
                        except Exception:
                    ...
                    # Prepare even tighter next round (respect floors) - only if we're continuing
                    if color_idx < len(color_attempts) - 1:
                        # For next color, slightly reduce width/fps
                        attempt_width = max(2, int((attempt_width * 0.95) // 2 * 2))
                        attempt_fps = max(6, int(attempt_fps * 0.95))
                        if enforce:
                            if min_w > 0 and attempt_width < min_w:
                                attempt_width = int(min_w // 2 * 2)
                            if min_fps > 0 and attempt_fps < min_fps:
                ...
                # Update current_bytes after all iterations
                if os.path.exists(gif_path):
                    current_bytes = os.path.getsize(gif_path)
                    current_size_mb = current_bytes / 1024 / 1024
                    logger.debug(f"[Iterative Tightening] Completed {iteration_count} iterations, final size: {current_size_mb:.2f}MB (target: {target_size_mb:.2f}MB)")
                ...
                try:
                    if os.path.getsize(backup_path) <= target_bytes:
                        try:
                            ...
                            logger.warning(f"Failed to restore original GIF: {restore_e}")
                except Exception:
                    pass

            # Final summary log
            final_bytes = os.path.getsize(gif_path) if os.path.exists(gif_path) else original_bytes
            final_size_mb = final_bytes / 1024 / 1024
            logger.warning(f"GIF optimization did not meet target: final={final_size_mb:.2f}MB, target={target_size_mb:.2f}MB")
            logger.info(f"Optimization summary: {len(optimization_attempts)} attempts performed")
            for i, attempt in enumerate(optimization_attempts, 1):
                stage = attempt.get('stage', 'Unknown')
                result_size = attempt.get('result_size_mb', 'N/A')
                ...
                logger.info(f"  {i}. {status} {stage}: {result_size}MB{params_str}{error_str}")
            return False

        except Exception as e:
            logger.error(f"Error optimizing GIF: {e}")
            # Clean up any remaining temp files on exception
            try:
                # Clean up common temp file patterns
                import glob
                temp_patterns = [
                    gif_path + ".lossless.tmp.gif",
                    gif_path + ".near.tmp.gif",
                    gif_path + ".ffmpeg.tmp.gif",
                    gif_path + ".tight_*.tmp.gif"
                ]
    ...
    def _ffmpeg_palette_reencode(self, input_path: str, output_path: str, new_width: int, fps: int, max_colors: int = 256) -> bool:
        """Re-encode GIF via ffmpeg using mpdecimate + palettegen/paletteuse preserving AR with file-based caching."""
        try:
            if self._shutdown_checker():
                return False
            
            # Log input file size and parameters
            input_size_mb = 0
            if os.path.exists(input_path):
                input_size_mb = os.path.getsize(input_path) / 1024 / 1024
            logger.debug(f"[FFmpeg Re-encode] Input: {input_size_mb:.2f}MB, params: width={new_width}, fps={fps}, colors={max_colors}")
            
            # Use cache key for palette based on input GIF parameters
            # Get input file mtime for cache invalidation
            try:
            ...
            if palette_cached:
                logger.debug(f"[FFmpeg Re-encode] Using cached palette: {palette_path}")
            else:
                logger.debug(f"[FFmpeg Re-encode] Generating new palette: {palette_path}")
            ...
            if not palette_cached:
                ...
                if r1.returncode != 0 or not os.path.exists(palette_path):
                    try:
                        ...
                        pass
                    return False
                logger.debug(f"Cached palette (reencode): {palette_path}")
            ...
            if os.path.exists(palette_path):
                if not palette_path.startswith(self.palette_cache_dir):
            ...
            # Log output file size and result
            success = r2.returncode == 0 and os.path.exists(output_path)
            if success:
                output_size_mb = os.path.getsize(output_path) / 1024 / 1024 if os.path.exists(output_path) else 0
                reduction = ((input_size_mb - output_size_mb) / input_size_mb * 100) if input_size_mb > 0 else 0
                logger.debug(f"[FFmpeg Re-encode] Output: {output_size_mb:.2f}MB, reduction: {reduction:.1f}%")
            else:
                logger.debug(f"[FFmpeg Re-encode] Failed: returncode={r2.returncode}, output_exists={os.path.exists(output_path)}")
            return success
        except Exception as e:
            if self._shutdown_checker():
                logger.info("ffmpeg palette re-encode interrupted by shutdown")
            else:
                logger.debug(f"[FFmpeg Re-encode] Exception: {e}")
            return False
    ...
    def _apply_ffmpeg_smart_strategy(self, input_path: str, output_path: str, params: Dict[str, Any]) -> bool:
        """Apply smart FFmpeg-based optimization strategy with file-based caching"""
        
        # Use cache key for palette based on input parameters
        # Get input file info for cache key
        try:
            from ..ffmpeg_utils import FFmpegUtils
            ffmpeg_utils = FFmpegUtils()
            video_info = ffmpeg_utils.get_video_info(input_path) or {}
            width = video_info.get('width', params.get('width', 480))
            height = video_info.get('height', params.get('height', 360))
        except Exception:
        ...
        palette_cached = os.path.exists(palette_path)
        if palette_cached:
            logger.debug(f"Using cached palette (smart): {palette_path}")
        ...
        if not palette_cached:
            ...
            if (self.shutdown_requested or self._shutdown_checker()) or palette_result.returncode != 0:
                return False
            logger.debug(f"Cached palette (smart): {palette_path}")
    ...
    def _apply_ffmpeg_resize_strategy(self, input_path: str, output_path: str, params: Dict[str, Any]) -> bool:
        """Apply FFmpeg-based resize optimization strategy with file-based caching"""
        ...
        palette_cached = os.path.exists(palette_path)
        if palette_cached:
            logger.debug(f"Using cached palette (resize): {palette_path}")
        ...
        if not palette_cached:
            ...
            if (self.shutdown_requested or self._shutdown_checker()) or palette_result.returncode != 0:
                return False
            logger.debug(f"Cached palette (resize): {palette_path}")
    ...
    def _apply_ffmpeg_adaptive_strategy(self, input_path: str, output_path: str, params: Dict[str, Any]) -> bool:
        """Apply adaptive FFmpeg-based optimization strategy with file-based caching"""
        ...
        palette_cached = os.path.exists(palette_path)
        if palette_cached:
            logger.debug(f"Using cached palette (adaptive): {palette_path}")
        ...
        if not palette_cached:
            ...
            if (self.shutdown_requested or self._shutdown_checker()) or palette_result.returncode != 0:
                return False
            logger.debug(f"Cached palette (adaptive): {palette_path}")
    ...
    def _apply_ffmpeg_upscale_strategy(self, input_path: str, output_path: str, params: Dict[str, Any]) -> bool:
        """Apply FFmpeg-based upscaling optimization strategy with file-based caching"""
        ...
        palette_cached = os.path.exists(palette_path)
        if palette_cached:
            logger.debug(f"Using cached palette (upscale): {palette_path}")
        ...
        if not palette_cached:
            ...
            if (self.shutdown_requested or self._shutdown_checker()) or palette_result.returncode != 0:
                return False
            logger.debug(f"Cached palette (upscale): {palette_path}")
        ...
        try:
            ...
            for t in tiers:
                try:
                    ...
                    # Use cache key for palette based on input GIF parameters
                    palette_path = self._get_palette_cache_key(
                        input_video=gif_path,
                        method='reencode_tier',
                        width=sw,
                        height=sh,
                        fps=t["fps"],
                        colors=t["colors"]
                    ...
                    palette_cached = os.path.exists(palette_path)
                    if palette_cached:
                        logger.debug(f"Using cached palette (reencode_tier): {palette_path}")
                    ...
                    if not palette_cached:
                        ...
                        if palette_result.returncode != 0 or not os.path.exists(palette_path):
                            continue
                        logger.debug(f"Cached palette (reencode_tier): {palette_path}")
    ...
    def _generate_palette_neuquant(self, input_video: str, colors: int) -> Optional[str]:
        """Generate palette using NeuQuant algorithm (via FFmpeg) with file-based caching."""
        # Use cache key based on input parameters
        palette_file = self._get_palette_cache_key(
        ...
        if os.path.exists(palette_file):
            logger.debug(f"Using cached palette (neuquant): {palette_file}")
            return palette_file
        ...
        try:
            ...
            if result.returncode == 0 and os.path.exists(palette_file):
                logger.debug(f"Cached palette (neuquant): {palette_file}")
                return palette_file
        except Exception:
            pass
    ...
    def _generate_palette_median_cut(self, input_video: str, colors: int) -> Optional[str]:
        """Generate palette using median cut algorithm with file-based caching"""
        # Use cache key based on input parameters
        palette_file = self._get_palette_cache_key(
            input_video=input_video,
            method='mediancut',
        ...
        # Return cached palette if exists
        if os.path.exists(palette_file):
            logger.debug(f"Using cached palette (median_cut): {palette_file}")
            return palette_file
        
        # Ensure cache dir exists (before any file operations)
        try:
            os.makedirs(self.palette_cache_dir, exist_ok=True)
            if not os.path.exists(self.palette_cache_dir):
                logger.error(f"Failed to create palette cache directory: {self.palette_cache_dir}")
                return None
        except Exception as e:
            logger.error(f"Failed to create palette cache directory {self.palette_cache_dir}: {e}")
            return None
            ...
            if result.returncode == 0:
                ...
                if sample_files:
                    ...
                    if all_pixels:
                        ...
                        # Verify cache dir exists before saving
                        if not os.path.exists(self.palette_cache_dir):
                            logger.error(f"Palette cache directory does not exist: {self.palette_cache_dir}")
                            return None
                        ...
                        for retry in range(max_retries):
                            try:
                                ...
                                if os.path.exists(palette_file):
                                    logger.debug(f"Cached palette (median_cut): {palette_file}")
                                    return palette_file
                                break
                            except (PermissionError, OSError) as e:
                                if retry < max_retries - 1:
    ...
    def _generate_palette_octree(self, input_video: str, colors: int) -> Optional[str]:
        """Generate palette using octree algorithm with file-based caching"""
        # Use cache key based on input parameters
        palette_file = self._get_palette_cache_key(
            input_video=input_video,
            method='octree',
        ...
        # Return cached palette if exists
        if os.path.exists(palette_file):
            logger.debug(f"Using cached palette (octree): {palette_file}")
            return palette_file
        ...
        for retry in range(max_retries):
            try:
                ...
                if result.returncode == 0 and os.path.exists(palette_file):
                    logger.debug(f"Cached palette (octree): {palette_file}")
                    return palette_file
