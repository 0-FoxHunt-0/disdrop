...
class ResolutionAwareQualityEvaluator:
    ...
    def _compute_ssim_direct(self, ref_path: str, dist_path: str) -> Tuple[Optional[float], float]:
        ...
        try:
            ...
            if result.returncode == 0:
                return self._parse_ssim_output_simple(result.stderr)
            else:
                logger.error(f"Direct SSIM computation failed: {result.stderr}")
                return None, 0.0
                
        except Exception as e:
            logger.error(f"Direct SSIM computation error: {e}")
            return None, 0.0
    ...
    def _parse_ssim_output_simple(self, stderr: str) -> Tuple[Optional[float], float]:
        ...
        try:
            ...
            logger.debug(f"SSIM parsing error: {e}")
            return None, 0.0
